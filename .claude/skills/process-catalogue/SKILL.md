...
- Path to `refrences/Master.csv` (match.py builds/reads `Master.csv.db` SQLite automatically)
...
### Step 3 — Recovery Pass for Unmatched

Load `references/accuracy-loop-guide.md` for full rules.

**Semantic Mapping:**
For any rows where `Match Type = UNMATCHED`, apply LLM medical knowledge to recover matches:
...
- Search **both** provider names AND catalogue names in `Master.csv.db` with `token_sort_ratio` threshold ≥ 65%
